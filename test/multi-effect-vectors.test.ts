  ...
  for (const v of fixture.vectors.filter((vec) => APPLICABLE.has(vec.surface))) {
    it(`${v.id}: requires ${sortedKeys(v.requires).join(" + ") || "nothing"}`, () => {
      ...
      if (v.requires.length === 0) {
        // Either refusal satisfies the canon (no pair charged); PKA-159 makes
        // an inexpressible ARGUMENT the hard-deny kind specifically.
        assert.ok(mapped === null || isParseUnsafe(mapped), `${v.id}: must not be mapped`);
        return;
      }
      assert.ok(mapped !== null && !isParseUnsafe(mapped), `${v.id}: expected a mapping`);
