...
import { mapToolCall, isParseUnsafe, PARSE_UNSAFE } from "../src/mapper.js";
...
describe("mapToolCall — filesystem tools", () => {
  ...
  it("move_file requires BOTH endpoints resolvable (never move an ambiguous target)", () => {
    // PKA-159: a recognised tool with an inexpressible argument is PARSE_UNSAFE
    // (hard deny), not null (unknown tool → operator escape hatch).
    assert.deepEqual(mapToolCall("move_file", { destination: "/b/final.txt" }), PARSE_UNSAFE);
    assert.deepEqual(mapToolCall("move_file", { source: "/a" }), PARSE_UNSAFE);
    assert.deepEqual(mapToolCall("move_file", { source: 1, destination: "/b" }), PARSE_UNSAFE);
    // PKA-157: in-bounds traversal now FOLDS instead of refusing.
    assert.deepEqual(mapToolCall("move_file", { source: "/a/x/../draft", destination: "/b/f" }), {
      action: "write",
      resource: "fs/b/f",
      alsoRequires: [
        { action: "read", resource: "fs/a/draft" },
        { action: "delete", resource: "fs/a/draft" },
      ],
    });
    // ...but an escaping one still refuses, as a hard deny.
    assert.deepEqual(mapToolCall("move_file", { source: "/a/../../x", destination: "/b" }), PARSE_UNSAFE);
  });
  ...
  it("read_multiple_files with only non-sensitive paths maps one read per path (PKA-148)", () => {
    ...
    assert.deepEqual(mapToolCall("read_multiple_files", {}), PARSE_UNSAFE);
    assert.deepEqual(mapToolCall("read_multiple_files", { paths: [] }), PARSE_UNSAFE);
  });
  ...
  it("folds in-bounds traversal, refuses only escapes (PKA-157/159/190)", () => {
    // Refusing EVERY `.`/`..` was the old behaviour. It rejected ordinary
    // relative paths like `./x`, and operators reached for
    // `unmapped_verdict: "allow"` to get work done — which is exactly how the
    // refusal turned into the PKA-159 bypass. Folding removes that pressure.
    assert.deepEqual(mapToolCall("write_file", { path: "./x" }), { action: "write", resource: "fs/x" });
    assert.deepEqual(mapToolCall("read_text_file", { path: "/a/./b" }), { action: "read", resource: "fs/a/b" });
    assert.deepEqual(mapToolCall("read_text_file", { path: "/a/b/../c" }), { action: "read", resource: "fs/a/c" });
    // One file, one resource, whichever spelling the caller used.
    assert.deepEqual(
      mapToolCall("read_text_file", { path: "/a/x/../b" }),
      mapToolCall("read_text_file", { path: "/a/b" }),
    );
    // Folding runs BEFORE the sensitive overlay, so traversal cannot dodge it.
    assert.deepEqual(mapToolCall("read_text_file", { path: "/home/u/x/../.env" }), {
      action: "read",
      resource: "secret/env",
    });

    // Escapes still refuse — and as a HARD deny, not an unknown-tool result.
    // NOTE: `/safe/../etc/shadow` is NOT here. It folds to `etc/shadow`, which
    // is exactly the file the call reaches, so `fs/etc/shadow` is the correct
    // resource to ask the PDP about. Refusing it would be the over-strictness
    // this change exists to remove. Only escapes ABOVE the root refuse.
    for (const path of ["..", "/a/../..", "../../x", "/.."]) {
      const got = mapToolCall("read_text_file", { path });
      assert.ok(isParseUnsafe(got), `${path} must be PARSE_UNSAFE, got ${JSON.stringify(got)}`);
    }
  });

  it("refuses missing / non-string / empty / NUL paths as a hard deny", () => {
    for (const args of [{}, { path: 42 }, { path: "" }, { path: "/a\0b" }, { path: "///" }, { path: "." }]) {
      assert.ok(isParseUnsafe(mapToolCall("read_text_file", args)), JSON.stringify(args));
    }
  });
...
describe("mapToolCall — filesystem sensitive-path overlay (secret/<kind>)", () => {
  ...
  it("normal paths still map to fs/<path> (overlay is a strict overlay)", () => {
    ...
    for (const path of ["/home/u/.ssh", "/home/u/.envrc", "/home/u/.environment", "/x/.ssh/nested/id_rsa", "/x/.aws/credentials/extra"]) {
      const got = mapToolCall("read_text_file", { path });
      assert.ok(got !== null && !isParseUnsafe(got) && got.resource.startsWith("fs/"), `${path}`);
    }
  });
  ...
  it("folding precedes the overlay, so traversal cannot dodge secret classification", () => {
    // Was: both of these refused. Refusing was safe but blunt — and it refused
    // `./.env`, an ordinary spelling. Now they FOLD and the overlay still
    // claims them, which is the property that actually matters: you cannot
    // reach a credential under an fs/ resource by spelling the path differently.
    assert.deepEqual(mapToolCall("read_text_file", { path: "/safe/../.env" }), {
      action: "read",
      resource: "secret/env",
    });
    assert.deepEqual(mapToolCall("write_file", { path: "./.env" }), {
      action: "write",
      resource: "secret/env",
    });
  });
  ...
  it("move_file: sensitive source wins the primary pair; both endpoints fully enforced", () => {
    ...
    // PKA-157: `/b/../x` is in-bounds — it folds to `x`, so the move maps and
    // both endpoints are still fully enforced. Only an ESCAPE refuses.
    assert.deepEqual(mapToolCall("move_file", { source: "/home/u/.env", destination: "/b/../x" }), {
      action: "write",
      resource: "secret/env",
      alsoRequires: [
        { action: "read", resource: "secret/env" },
        { action: "delete", resource: "secret/env" },
        { action: "write", resource: "fs/x" },
      ],
    });
    assert.ok(isParseUnsafe(mapToolCall("move_file", { source: "/../x", destination: "/home/u/.env" })));
  });
  ...
  it("read_multiple_files: every member is required, the first secret keeps the primary (PKA-148)", () => {
    ...
    // PKA-157: `/p/../x` folds in-bounds to `x`, so the list still maps.
    assert.deepEqual(mapToolCall("read_multiple_files", { paths: ["/p/../x", "/home/u/.env"] }), {
      action: "read",
      resource: "secret/env",
      alsoRequires: [{ action: "read", resource: "fs/x" }],
    });
    // A member that normalizes away, escapes, or is not a string refuses the
    // WHOLE call — as a hard deny (PKA-159), never the unmapped escape hatch.
    for (const paths of [["/p/a.ts", "//", "/home/u/.env"], ["/home/u/.env", 42], ["/p/a.ts", "/../x"]]) {
      assert.ok(isParseUnsafe(mapToolCall("read_multiple_files", { paths })), JSON.stringify(paths));
    }
  });
});
