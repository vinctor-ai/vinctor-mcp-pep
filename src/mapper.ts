...
function mapFilesystem(tool: string, args: Record<string, unknown>): MappedCall | null {
  ...
  if (tool === "move_file") {
    ...
    if (sourceRes === null || destinationRes === null) return null;
...
type Family = {
  ...
  readonly map: (tool: string, args: Record<string, unknown>) => MappedCall | null;
};
...
export function mapToolCall(name: string, args: Record<string, unknown>): MappedCall | null {
